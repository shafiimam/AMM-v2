...
var Player = class extends EventTarget {
  constructor(durationInSec, stopOnVisibility = true) {
    ...
    __privateAdd(
      this,
      _onVisibilityChangeListener,
      __privateMethod(this, _Player_instances, onVisibilityChange_fn).bind(this)
    );
...
var LoadingBar = class extends HTMLElement {
  constructor() {
    ...
    document.addEventListener(
      "theme:loading:start",
      __privateMethod(this, _LoadingBar_instances, onLoadingStart_fn).bind(this)
    );
    document.addEventListener(
      "theme:loading:end",
...
setupListeners_fn2 = function () {
  if (this.allCells.length > 1) {
    ...
    this.addEventListener(
      "scroll",
      throttle(
        __privateMethod(this, _ScrollCarousel_instances, onScroll_fn).bind(this)
      )
    );
    this.addEventListener(
      "scrollend",
...
var QuantityInput = class extends HTMLElement {
  constructor() {
    ...
    __privateGet(
      ...
      "change",
      __privateMethod(this, _QuantityInput_instances, onValueChange_fn).bind(
        this
      )
    );
    __privateGet(
...
onSwatchChanged_fn = async function (event, target) {
  if (target.hasAttribute("data-product-url")) {
    this.querySelectorAll(`a[href^="${Shopify.routes.root}products/"`).forEach(
      (link) => {
          ...
          if (matchingBlock) {
            if (blockType === "buy-buttons") {
              ...
              if (quantitySelectorElement) {
                ...
                matchingBlock.querySelector("quantity-selector").quantity =
                  existingQuantity;
              }
            } else {
              element.replaceWith(matchingBlock);
...
var QuickBuyModal = class extends Modal {
  constructor() {
    ...
    this.addEventListener(
      "dialog:after-hide",
      __privateMethod(this, _QuickBuyModal_instances, onAfterHide_fn).bind(this)
    );
  }
  async show() {
    document.documentElement.dispatchEvent(
...
var BeforeAfter = class extends HTMLElement {
  constructor() {
    ...
    __privateAdd(
      ...
      __privateMethod(this, _BeforeAfter_instances, onPointerMove_fn).bind(this)
    );
